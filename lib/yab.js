...
if (args[0] === 'this') {
	log('Creating a new scaffolding of the current directory by the name of `' + args[1] + '`.')
...
if (args[0] === 'list') {
	log('These are the current scaffoldings saved in yab.')
...
if (args[0] === 'remove') {
	log('Removed the scaffolding `' + args[1] + '`.')
	fs.unlinkSync(templateDir + '/' + args[1]);
	return
}

if (! loadTemplate(args[0])) {
	log('That scaffolding does not exists.  Create it with `yab new [name]`.')
	return
}
